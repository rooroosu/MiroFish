...
// Card layout config — adjusted to wider proportions
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute number of cards in the current row to center each row
    ...
    const x = offset * 35
    // Adjust start position: close to title but keep appropriate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Clear previous debounce timer (new scroll intent overwrites old one)
        ...
        // Use debounce delay for state transitions to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether pending state still needs to execute (may be overwritten by later scroll)
          ...
          setTimeout(() => {
            ...
            // After animation ends, check whether there is a new pending state
...
// Reload data when component activates (if using keep-alive)
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Positioned from top-left; height changes extend only at bottom without shifting existing grid */
