...
// Format simulated elapsed time (based on round number and minutes-per-round)
...
// Reset all state (for re-starting simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
// Track last round per platform to detect changes and emit logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation is completed (via runner_status or platform completion)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: backend may not yet update runner_status, but platforms already reported complete
...
const checkPlatformsCompleted = (data) => {
  ...
  // Return false if no platforms were enabled
