    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Rounds config — only shown when config generation is done and rounds are computed -->
...
// Log deduplication: record the last output key info
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended) to avoid slider range anomalies
...
// Compute total number of associated topics across all profiles
...
const startPrepareSimulation = async () => {
  ...
  // Mark first step done; start second step
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected agent total (obtained from prepare API response)
...
const loadPreparedData = async () => {
  ...
  // Get config (using realtime endpoint)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
