...
const renderGraph = () => {
  ...
  // Process edge data: count edges and index between each node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Determine whether edge direction matches normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Compute curvature: spread apart for multiple edges, straight for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // This ensures all edges distribute under the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150; add 40 per extra edge
    ...
    // Add center-attraction force so isolated node clusters gravitate toward the center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw a circular arc that leaves and returns to the node
      const loopRadius = 30
    ...
    // Perpendicular offset to the connection, scaled by distance ratio to keep curve visible
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first confirmed drag
          d._isDragging = true
