...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract analysis question — bilingual zh/en
    ...
    // Extract statistics — match '相关预测事实: X条' or 'Related facts: X' format
    const factMatch = text.match(/(?:相关预测事实|Related (?:prediction )?facts)[:：]\s*(\d+)/i)
    const entityMatch = text.match(/(?:涉及实体|Entities involved|Related entities)[:：]\s*(\d+)/i)
    const relMatch = text.match(/(?:关系链|Relation(?:ship)? chains?)[:：]\s*(\d+)/i)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    // Extract sub-questions — bilingual heading
    ...
    // Extract key facts — bilingual heading; tolerate 【】 brackets or plain
    ...
    // Extract core entities — bilingual heading
    ...
    // Extract relationship chains — bilingual heading
    const relSection = text.match(/### (?:【关系链】|Relation(?:ship)? Chains?|\[Relation(?:ship)? Chains?\])\n([\s\S]*?)(?=\n###|$)/i)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/(?:当前有效事实|Current valid facts|Active facts)[:：]\s*(\d+)/i)
    ...
    const activeSection = text.match(/### (?:【当前有效事实】|Current Valid Facts|Active Facts|\[Active Facts\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (activeSection) {
    ...
    const entitySection = text.match(/### (?:【涉及实体】|Entities Involved|\[Entities Involved\])\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*(?:采访人数|Interview count|Interviews)[:：]\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:采访对象选择理由|Interviewee Selection Rationale|Selection Reason)\n([\s\S]*?)(?=\n---\n|\n### (?:采访实录|Interview Transcript|Interviews))/i)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1: digit. **Name（index=X）**：rationale
        // Example: 1. **Alumni_345（index=1）**：as a Wuhan Univ alumnus...
        ...
        if (name) {
          ...
          // Continuation of rationale (excluding closing summary paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // Save the last person's rationale
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling when first question is preceded by '1.'
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:关键引言|Key Quote)|$)/i)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/(?:【Twitter平台回答】|\[Twitter response\]|\[Twitter Reply\])\n?([\s\S]*?)(?=(?:【Reddit平台回答】|\[Reddit response\]|\[Reddit Reply\])|$)/i)
        const redditMatch = answerText.match(/(?:【Reddit平台回答】|\[Reddit response\]|\[Reddit Reply\])\n?([\s\S]*?)$/i)
        ...
        // Platform fallback logic (for old format: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer — copy as default display only when non-placeholder
          ...
          // No per-platform markers (very old format) — use entire text as answer
    ...
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary(?: and Core Insights?)?|Summary and Key Insights?)\n([\s\S]*?)$/i)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Attempt to extract edge info (if available)
    ...
    // Attempt to extract node info (if available)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain separate platform selection state for each Q&A pair
    ...
    const isPlaceholderText = (text) => {
      ...
      const noResp = t('step4.platformNoResponse')
      return trimmed === '（该平台未获得回复）' || trimmed === '(该平台未获得回复)'
        || trimmed === '[无回复]' || trimmed === noResp || trimmed === `(${noResp})`
    }

    // Attempt to split answer by question numbering
    ...
    // Get answer corresponding to a given question
    ...
    // Check whether a given question has dual-platform answers (filtering placeholders)
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - one question per exchange
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (shown only when both platforms have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 heading (## xxx) since section title is already shown in outer layer
  ...
  // Strip whitespace after list open tag
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Strip whitespace before list close tag
  ...
  // Clean up <p><br> immediately before block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph open tag that immediately follows a block-level element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: keep counter incrementing when single-item <ol> is separated by paragraph content
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled uniformly in the post-loop nextTick
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Scroll to top if task complete; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Attempt to extract content inside <final_answer> tags
  ...
  // Attempt to find content after '最终答案:' (Chinese)
