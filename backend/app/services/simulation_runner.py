...
Runs simulations in background processes, records each agent's actions, and supports real-time status monitoring
"""
...
class SimulationRunState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform independent round counts and times
...
class SimulationRunner:
    ...
    _stdout_files: Dict[str, Any] = {}  # stored stdout file handles
    _stderr_files: Dict[str, Any] = {}  # stored stderr file handles
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # max simulation rounds (optional, to cap overly long simulations)
        ...
        Args:
            ...
            max_rounds: max simulation rounds (optional, to cap overly long simulations)
        ...
        if enable_graph_memory_update and Config.is_local_memory_backend():
            logger.info("local_qmd backend is active; graph memory update is disabled")
            cls._graph_memory_enabled[simulation_id] = False
        elif enable_graph_memory_update:
            if not graph_id:
        ...
        try:
            ...
            # If max_rounds specified, add as CLI arg
            if max_rounds is not None and max_rounds > 0:
            ...
            # Set working dir to simulation dir (databases etc. are created here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # new process group so all children can be killed on server shutdown
            )
            
            # Save file handle for later cleanup
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # Process ended — do one final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If both are running, both must complete
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update per-platform round counts and times
                                    if platform == "twitter":
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force kill, /T = kill tree (including children)
            ...
            # Since start_new_session=True, process group ID == main process PID
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Platform: prefer record's platform field, fall back to default
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform auto-set to "twitter" by file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (platform auto-set to "reddit" by file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files not found, try reading old single-file format
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (to force a restart).
        ...
        # Directories to delete (contain action logs)
        ...
        if simulation_id in cls._run_states:
            ...
            logger.info(f"Simulation log cleanup complete: {simulation_id}, deleted: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean up (avoid printing useless logs for empty process set)
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in the reloader child process (the one actually running the app)
        # WERKZEUG_RUN_MAIN=true means it is the reloader child process
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: platform to target (optional)
                - "twitter": Twitter only
                - "reddit": Reddit only
                - None: both platforms simultaneously (returns merged result)
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: simulation ID
            interviews: list of interviews, each {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: default platform (optional; overridden per interview item)
                ...
                - None: both platforms simultaneously per agent
            timeout: timeout in seconds
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation to gracefully exit command-waiting mode.
        ...
        try:
            ...
            # Timeout may be because the environment is already closing
            return {
                "success": True,
