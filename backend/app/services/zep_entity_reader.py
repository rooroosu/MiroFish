...
Reads nodes from the Zep graph and filters for nodes matching predefined entity types
"""
...
class ZepEntityReader:
    ...
    2. Filter for nodes matching predefined entity types (nodes whose Labels are not only "Entity")
    ...
    def _call_with_retry(
        ...
        Args:
            func: function to execute (no-argument lambda or callable)
            operation_name: operation name (for logging)
            max_retries: maximum retries (default 3; at most 3 attempts)
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all nodes in a graph (paginated).
        ...
        if self.local_store:
            return self.local_store.get_nodes(graph_id)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges in a graph (paginated).
        ...
        if self.local_store:
            return self.local_store.get_edges(graph_id)
    ...
    def filter_defined_entities(
        ...
        """
        Filter for nodes matching predefined entity types.

        Filtering logic:
        - If a node's Labels contain only "Entity", it does not match our predefined types — skip it
        - If a node's Labels contain labels other than "Entity" and "Node", it matches — keep it
        ...
        # Fetch all edges (for subsequent relation lookup)
        ...
        for node in all_nodes:
            ...
            if enrich_with_edges:
                ...
                # Fetch basic info for connected nodes
                related_nodes = []
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity and its full context (edges and connected nodes, with retry).
        ...
        """
        try:
            # Use retry mechanism to fetch node
