...
2. Optimized prompts for highly detailed persona generation
...
class OasisProfileGenerator:
    ...
    Converts Zep graph entities into Agent Profiles required by the OASIS simulation.

    Key features:
    1. Uses Zep graph retrieval to enrich context
    2. Generates highly detailed personas (including background, career, personality, social media behavior)
    3. Distinguishes individual entities from abstract group entities
    """
    ...
    # Group/institution entity types (require representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Retrieve rich entity-related information using Zep graph hybrid search.

        Zep does not have a built-in hybrid search API; edges and nodes are searched
        separately and results are merged. Uses parallel requests for efficiency.
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check whether this is a group/institution-type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        # Attempt generation multiple times until success or max retries
        max_attempts = 3
        last_error = None
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last quote is not followed by comma or closing bracket, the string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # Mark as fixed if meaningful content was extracted
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation, faithfully reflecting existing real-world conditions as closely as possible. You must return valid JSON; all string values must not contain unescaped newline characters."
    ...
    def _build_individual_persona_prompt(
        ...
        """Build detailed individual entity persona prompt"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "N/A"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media user persona for the entity below, faithfully reflecting existing real-world conditions as closely as possible.
...
2. persona: Detailed persona description (2000-word plain text), including:
   ...
   - Stance and opinions (attitude toward topics, content that might provoke or move this person)
   - Distinctive traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona: describe this individual's connection to the events and their prior actions and reactions)
...
- persona must be a single cohesive text passage
- {get_language_instruction()} (the gender field must be English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed group/institution entity persona prompt"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "N/A"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for the institution/group entity below, faithfully reflecting existing real-world conditions as closely as possible.
...
2. persona: Detailed account profile description (2000-word plain text), including:
   - Institution basic information (official name, nature of institution, founding background, main functions)
   ...
   - Institutional memory (important part of the institution persona: describe this institution's connection to the events and its prior actions and reactions)
...
- persona must be a single cohesive text passage; do not use newline characters
- {get_language_instruction()} (the gender field must be English "other")
...
- Institutional account speech must be consistent with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        """
        Generate Agent Profiles from entities in bulk (supports parallel generation).
        ...
        completed_count = [0]  # use a list so it can be modified in a closure
        lock = Lock()
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Print the generated persona to console and log in real time
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print the generated persona to console in real time (full content, not truncated)"""
        separator = "-" * 70

        # Build full output (not truncated)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                # user_char: full persona (bio + persona), used as LLM system prompt
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Must include the user_id field — this is the key matched by OASIS agent_graph.get_agent()!
