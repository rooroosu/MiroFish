"""
Zep graph retrieval tool service.
Wraps graph search, node retrieval, edge queries, etc. Used by Report Agent.

Core retrieval tools (optimized):
1. InsightForge - most powerful hybrid retrieval, auto-generates sub-queries, multi-dimensional
2. PanoramaSearch - broad search, includes expired content
...
from .local_qmd_backend import LocalQMDGraphStore
...
class InsightForgeResult:
    ...
    Contains results from multiple sub-queries and a synthesized analysis.
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """Convert to text format (full, no truncation)."""
        text_parts = [
            f"## Broad Search Results (Full Panoramic View)",
            f"Query: {self.query}",
            f"\n### Statistics",
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out lines with question numbers (1-9)
                skip = False
                for d in '123456789':
...
class ZepToolsService:
    ...
    4. interview_agents - deep interview (interview simulated agents, multi-perspective)

    [Base tools]
    - search_graph - graph semantic search
    - get_all_nodes - fetch all graph nodes
    - get_all_edges - fetch all graph edges (with temporal info)
    - get_node_detail - fetch node detail
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        self.local_store = LocalQMDGraphStore() if Config.is_local_memory_backend() else None
        self.api_key = api_key or Config.ZEP_API_KEY
        if not self.local_store and not self.api_key:
            raise ValueError("ZEP_API_KEY not configured")
        
        self.client = None if self.local_store else Zep(api_key=self.api_key)
    ...
    def search_graph(
        ...
        if self.local_store:
            local_result = self.local_store.search(graph_id, query, limit, scope)
            facts = []
    ...
    def _local_search(
        ...
        """
        Local keyword-matching search (fallback when Zep Search API unavailable).

        Fetches all edges/nodes then matches locally.
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        if self.local_store:
            result = []
            for node in self.local_store.get_nodes(graph_id):
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        if self.local_store:
            result = []
            for edge in self.local_store.get_edges(graph_id):
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        if self.local_store:
            logger.warning("local_qmd get_node_detail requires graph context; returning None")
            return None
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all graph edges then filters for those connected to the given node.
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # Check if edge is connected to the node (as source or target)
    ...
    def get_entity_summary(
        ...
        """
        Get a relationship summary for a given entity.
    ...
    def get_simulation_context(
        ...
        # Filter entities with concrete types (exclude pure Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function. Auto-decomposes the query and retrieves multi-dimensionally:
        1. Use LLM to break the query into sub-questions
        ...
        5. Synthesize all results into deep insight

        Args:
            graph_id: graph ID
            query: user question
            simulation_requirement: simulation requirement description
            report_context: optional report context (improves sub-query accuracy)
        ...
        # Fetch details for all relevant entities (full, no limit)
        entity_insights = []
        node_map = {}  # used for relationship chain building
    ...
    def _generate_sub_queries(
        ...
        Decomposes a complex question into independently retrievable sub-questions.
        """
        system_prompt = """You are a professional query analysis expert. Your task is to decompose a complex question into multiple sub-questions that can be independently observed in the simulation world.
...
Return a JSON list of sub-questions."""
    ...
    def panorama_search(
        ...
        Retrieves a full panoramic view including all related content and historical/expired info:
        1. Fetch all relevant nodes
        2. Fetch all edges (including expired/invalidated)
        3. Classify active vs historical facts

        Suitable for understanding the full picture of an event or tracking its evolution.
        ...
        for edge in all_edges:
            ...
            if is_historical:
                # Historical/expired fact, attach time markers
    ...
    def quick_search(
        ...
        3. Suitable for simple, direct queries
    ...
    def interview_agents(
        ...
        Calls the real OASIS interview API to interview agents currently running in a simulation:
        1. Auto-reads persona files to learn about all simulated agents
        2. Uses LLM to analyze interview requirements and intelligently select relevant agents
        ...
        5. Integrates all results and generates an interview report

        [Important] Requires the simulation environment to be running (OASIS env not closed).

        [Use cases]
        - Need multi-role perspectives on an event
        - Need to gather opinions from multiple parties
        ...
        Args:
            ...
            interview_requirement: interview requirement description (unstructured, e.g. "learn student opinions on the event")
            simulation_requirement: simulation background (optional)
            max_agents: max number of agents to interview
            custom_questions: custom questions (optional; auto-generated if not provided)
        ...
        # Step 4: Call real interview API (no platform specified = both platforms simultaneously)
        try:
            ...
            api_result = SimulationRunner.interview_agents_batch(
                ...
                timeout=180.0   # longer timeout needed for dual-platform
            )
            ...
            if not api_result.get("success", False):
                ...
                result.summary = f"Interview API call failed: {error_msg}. Check OASIS simulation environment status."
                return result
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Get this agent's results from both platforms
                ...
                # Strip possible tool-call JSON wrapper
                ...
                # Clean response text: strip markers, numbering, Markdown, etc.
                ...
                # Strategy 2 (fallback): text inside properly paired Chinese quotes 「」
                if not key_quotes:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Strip JSON tool-call wrapper from agent response and return the actual content."""
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # Convert CSV row to unified format
                        profiles.append({
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: full info of selected agents
                - selected_indices: indices of selected agents (for API call)
...
Select up to {max_agents} agents most suitable for this interview and explain your reasoning."""
        ...
        try:
            ...
            # Fetch full info for selected agents
            selected_agents = []
            valid_indices = []
...
4. Objective and neutral, without bias
...
- {quote_instruction}
- Bold (**keyword**) is OK; no other Markdown syntax"""
