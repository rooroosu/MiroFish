...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This is the usage required by the Zep SDK; the warnings come from dynamic class
        ...
        def safe_attr_name(attr_name: str) -> str:
            """Convert a reserved name to a safe name"""
    ...
    def add_text_batches(
        ...
        if self.local_store:
            if progress_callback:
                progress_callback("Writing local_qmd graph batches", 1.0)
            return self.local_store.add_text_batches(graph_id, chunks)
    ...
    def _wait_for_episodes(
        ...
        if self.local_store:
            if progress_callback:
                progress_callback("local_qmd graph batches are processed", 1.0)
            return
    ...
    def _get_graph_info(self, graph_id: str) -> GraphInfo:
        ...
        if self.local_store:
            nodes = self.local_store.get_nodes(graph_id)
            edges = self.local_store.get_edges(graph_id)
            entity_types = {
                label
                for node in nodes
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Get full graph data (with detailed information).

        Args:
            graph_id: graph ID

        Returns:
            dict containing nodes and edges, including timestamps, attributes, and other details
        """
        if self.local_store:
