...
INTERVIEW_PROMPT_PREFIX = "Based on your persona and all past memories and actions, reply to me directly in text without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview prompt by prepending a prefix to prevent the Agent from calling tools.
...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes matching predefined entity types (nodes with Labels other than just Entity).
    ...
    """
    try:
        if not Config.is_local_memory_backend() and not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - completed: run complete — preparation was done earlier
...
def prepare_simulation():
    """
    Prepare simulation environment (async task; all parameters generated by LLM).
    ...
    try:
        ...
        # Check whether preparation is already done (avoid duplicate generation)
        if not force_regenerate:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} is not yet prepared; starting prepare task")
        
        # Fetch required info from project
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id provided, check whether preparation is already complete first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task not found; if simulation_id provided, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates the reports directory, finds reports matching simulation_id,
...
def get_simulation_history():
    ...
    Used for home page history display. Returns simulation list enriched with
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get simulation config (reads simulation_requirement from simulation_config.json)
            ...
            # Get run state (reads user-configured actual rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-set total_rounds; fall back to recommended rounds if absent
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time inspection during generation
    - Returns extra metadata (e.g. file modification time, whether generation is in progress)
    - Can return partial info even before config generation completes
...
def generate_profiles():
    """
    Directly generate OASIS Agent Profiles from the graph (without creating a simulation).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // optional: dynamically update Agent activities to Zep graph memory
            "force": false                         // optional: force restart (stops running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, if the simulation is running or completed, it will first be stopped and run logs cleared
    ...
    try:
        ...
        if enable_graph_memory_update and Config.is_local_memory_backend():
            logger.info("local_qmd backend is active; ignoring enable_graph_memory_update")
...
def get_agent_stats(simulation_id: str):
    ...
    For frontend Agent activity ranking and action distribution display.
    """
    try:
...
def interview_agent():
    ...
    Note: this feature requires the simulation environment to be in running state (entered wait-for-command mode after completing the simulation loop).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // required
            "agent_id": 0,                     // required, Agent ID
            "prompt": "What is your take on this?",  // required, interview question
    ...
    Returns (no platform specified, dual-platform mode):
        {
            ...
            "data": {
                "agent_id": 0,
                "prompt": "What is your take on this?",
                "result": {
                    "agent_id": 0,
                    "prompt": "...",
                    "platforms": {
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think of B?"  // omit platform to use the default
                }
            ],
    ...
    try:
        ...
        # Optimize each interview item's prompt by adding prefix to prevent Agent tool calls
...
def interview_all_agents():
    ...
    try:
        ...
        # Optimize prompt by adding prefix to prevent Agent tool calls
        optimized_prompt = optimize_interview_prompt(prompt)
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation, causing it to exit the wait-for-command mode gracefully.

    Note: this differs from the /stop endpoint. /stop force-terminates the process;
    this endpoint lets the simulation close the environment and exit gracefully.
