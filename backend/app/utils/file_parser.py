...
def _is_translation_cache_file(path: str) -> bool:
    """`foo.en.md`, `foo.en.txt` -> True. Treat as already-English; never re-translate."""
    p = Path(path)
    stem = p.stem
    return stem.endswith('.en')


def _maybe_translate(file_path: str, text: str) -> str:
    """Run text through the Korean-to-English translator if configured.

    Imported lazily so that file_parser stays importable without the LLM
    client (e.g. for unit tests or local-only flows).
    """
    if _is_translation_cache_file(file_path):
        return text
    try:
        from ..services.doc_translator import translate_file_if_needed
        return translate_file_if_needed(file_path, text)
    except Exception:
        return text
...
class FileParser:
    ...
    def _extract_from_pdf(file_path: str) -> str:
        ...
        joined = "\n\n".join(text_parts)
        return _maybe_translate(file_path, joined)
    
    @staticmethod
    def _extract_from_md(file_path: str) -> str:
        """Extract text from Markdown with auto encoding detection"""
        return _maybe_translate(file_path, _read_text_with_fallback(file_path))
