...
Reads parameters from config file to run simulation fully automatically.
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        """
        route = resolve_model_route(config_model=self.config.get("llm_model"))
        apply_openai_compatible_env(route)

        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError("Missing API Key config — set LLM_API_KEY in project root .env file")

        print(f"LLM config: model={route.model}, base_url={route.base_url[:40] if route.base_url else 'default'}...")
...
def setup_signal_handlers():
    ...
    Allows the program to clean up resources (close DB, environment, etc.).
    """
