...
- After simulation completes, stays alive in command-waiting mode instead of immediately closing
- Supports receiving Interview commands via IPC
- Supports single-agent interviews and batch interviews
- Supports remote close-env command
...
if sys.platform == 'win32':
    ...
    # Force default encoding (affects open() default encoding).
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        Fixes third-party libraries (e.g. OASIS) that read files without specifying encoding.
        """
        # Only set default encoding for text mode (non-binary) when not already specified
...
from app.model_routing import apply_openai_compatible_env, resolve_model_route

try:
    apply_openai_compatible_env(resolve_model_route())
except ValueError:
    pass
...
# Add filter immediately at module load time, before any camel code runs
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize logging configuration for a simulation.
    ...
    # Clean up old log directory (if present)
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: target platform (optional)
                ...
                - None/unspecified: interview both platforms, return merged result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: default platform (can be overridden per interview item)
                - "twitter": interview only Twitter platform
                - "reddit": interview only Reddit platform
                - None/unspecified: each agent interviews both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        ...
        # Distribute both_platforms_interviews to each platform
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "Environment is closing"})
            return False
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database, enriched with full context.

    Args:
        db_path: path to database file
        last_rowid: maximum rowid processed last time (uses rowid instead of
    ...
    Returns:
        ...
        - actions_list: list of actions, each containing agent_id, agent_name,
          action_type, action_args (with context)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Enrich with context (post content, username, etc.)
...
def _enrich_action_context(
    ...
    """
    Enrich an action with context (post content, username, etc.).

    Args:
        cursor: database cursor
        action_type: action type
        action_args: action args (modified in place)
        agent_names: agent_id -> agent_name mapping
    """
    try:
        ...
        # Create comment: add the commented post's info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config for speed-up in parallel simulation:
    ...
    providers during parallel simulation, increasing concurrency capacity.

    Args:
        config: simulation config dict
        use_boost: whether to use boost LLM config (if available)
    """
    route = resolve_model_route(use_boost=use_boost, config_model=config.get("llm_model"))
    config_label = "[Boost LLM]" if use_boost else "[General LLM]"
    apply_openai_compatible_env(route)

    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key config — set LLM_API_KEY in project root .env file")

    print(f"{config_label} model={route.model}, base_url={route.base_url[:40] if route.base_url else 'default'}...")
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: max simulation rounds (optional, to truncate long simulations)
    ...
    # Fall back to OASIS default name for any agent not in config
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed database row (use rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end with actions_count=0 when no active agents
            if action_logger:
        ...
        # Fetch actually executed actions from DB and log them
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (falls back to general config if not available)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional, to truncate long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completes, skip command-waiting mode'
    )

    args = parser.parse_args()

    # Create shutdown event at the start of main so the whole program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to exit cleanly on SIGTERM/SIGINT.

    Persistent simulation scenario: stays alive after simulation completes,
    waiting for interview commands. On termination signal:
    1. Notify asyncio loop to exit waiting
    2. Allow the program to clean up resources (close DB, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        # Don't call sys.exit() directly — let asyncio loop exit normally and clean up.
